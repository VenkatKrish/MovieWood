...
protocol ZTPagingDelegate{
    func selectedVideoModel(movieInfo:Movies, indexVal:Int)
}
class ZTPagingTableViewCell: UITableViewCell, UIScrollViewDelegate {
    var delegate: ZTPagingDelegate? = nil
    ...
    func loadPagingScroll(videosVal:[Movies]? = [], delegateObj:ZTPagingDelegate){
