//
//  ZTChannelPayPerViewController.swift
//  ZettaOTT
//
...
class ZTChannelPayPerViewController: UIViewController {
...
extension ZTChannelPayPerViewController: UITableViewDelegate, UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.loadOrderDetails(dataVal: self.transactionsList?[indexPath.row], indexPath: indexPath, delegateVal: self, movieInfo : self.movieInfo)
...
extension ZTChannelPayPerViewController{
    func getOrders(isSpinnerNeeded:Bool = false){
        if NetworkReachability.shared.isReachable{
            ...
            ZTCommonAPIWrapper.movieOrdersUsingGET(movieId: self.movieInfo?.movieId ?? -1, pageNumber: self.pageNumber, pageSize: self.pageSize, sort: SortingStruct.sort_orderDateOn_desc.rawValue) { response, error in
                if isSpinnerNeeded == true{
...
extension ZTChannelPayPerViewController : btnDetailsDelegate{
    func btnDetailsTapped(btn: UIButton) {
        if let modelVal = self.transactionsList?[btn.tag]{
            Helper.shared.goToPayPerView(viewController: self, orderInfo: modelVal, playsInfo: nil, movieInfo:self.movieInfo)
        }
    }
}
