//
//  ZTStagePlaysViewController.swift
//  ZettaOTT
//
...
import Alamofire
import SwiftUI
class ZTStagePlaysViewController: UIViewController {
...
// API Methods
extension ZTStagePlaysViewController{
    func getStreamingNowMovies(){
        if NetworkReachability.shared.isReachable {
            ZTCommonAPIWrapper.streamNow(pageNumber: pageScrollMoviesPageNum, pageSize: pageScrollMoviesPageSize, showinios: "Y", sortSorted: true, contenttype: MovieSearchTag.streamNowStagePlays.rawValue, sort:SortingStruct.sort_streamDateOn_desc.rawValue) { (response, error) in
    ...
    func getWebSeriesVideos(isSpinnerNeeded:Bool){
            if NetworkReachability.shared.isReachable {
                ...
                ZTCommonAPIWrapper.searchMoviesGET(search: MovieSearchTag.stagPlays.rawValue, page: self.pageNumber, size: self.pageSize) { (response, error) in
                    if isSpinnerNeeded == true{
...
extension ZTStagePlaysViewController:UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout{
...
extension ZTStagePlaysViewController : ZTCollectionPagingDelegate{
