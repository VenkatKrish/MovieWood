...
extension ZTShortFilmsViewController{
    func getStreamingNowMovies(){
        if NetworkReachability.shared.isReachable {
            ZTCommonAPIWrapper.streamNow(pageNumber: pageScrollMoviesPageNum, pageSize: pageScrollMoviesPageSize, sortSorted: true, contenttype: MovieSearchTag.streamNowShortFilms.rawValue, sort:SortingStruct.sort_streamDateOn_desc.rawValue) { (response, error) in
    ...
    func getWebSeriesVideos(isSpinnerNeeded:Bool){
        ...
        if NetworkReachability.shared.isReachable {
            ZTCommonAPIWrapper.searchMoviesGET(search: MovieSearchTag.shortFilms.rawValue, page: 0, size: 100) { (response, error) in
